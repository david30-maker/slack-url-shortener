class ShortenedUrlsController < ApplicationController
    ...
    def show 
        @url = ShortenedUrl.find_by_short_url("EVN['APP_DOMAIN']/#{params[:short_url]}")
    ...
    def shorten(long_url)
        ...
        existing_url = ShortenedUrl.find_by_sanitize_url(sanitize_url: unique_url)
    ...
    def generate_short_url
      unique_id = [*'0'..'9',*'A'..'Z',*'a'..'z'].sample(6).join
      "EVN['APP_DOMAIN']#{unique_id}"
    end

    def send_short_url_to_slack(channel_id, short_url)
      Thread.new do
        client = Slack::Web::Client.new
        client.chat_postMessage(channel: channel_id, text: short_url, as_user: true)
      end
    end
end
